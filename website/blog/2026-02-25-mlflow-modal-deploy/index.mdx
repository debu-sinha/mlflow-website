...
I developed [`mlflow-modal-deploy`](https://pypi.org/project/mlflow-modal-deploy/), a community deployment plugin that adds [Modal](https://modal.com/)'s scale-to-zero GPU platform as a target in MLflow's plugin architecture. It's [listed in MLflow's official Community Plugins](https://mlflow.org/docs/latest/plugins.html#community-plugins) documentation and [documented in MLflow's deployment guides](https://mlflow.org/docs/latest/ml/deployment/deploy-model-to-modal/). The plugin targets teams that need GPU endpoints for workloads like bursty inference or rapid prototyping. A single `create_deployment()` call takes any `pyfunc` model from an MLflow experiment to a live, auto-scaling endpoint. The plugin handles dependency extraction, code generation, and GPU configuration automatically.
